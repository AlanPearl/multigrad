...
`multigrad` is a Python package which enables the implementation data-parallelized, differentiable models using the Jax framework.
Leveraging MPI (Message Passing Interface), `multigrad` efficiently sums summary statistics on processors across computing nodes,
...
In and beyond the fields of cosmology and astrophysics, parameterized models can describe complex systems, provided that the
parameters have been tuned adequately to fit the model to observational data. Fitting capabilities can be increased tremendously
...
`multigrad` addresses this gap by providing an easy-to-use interface for implementing data-parallelized models. It abstracts away
...
$L(\vec{y}(\vec{x}))$ where the summary statistics are summed over multiple processes: $\vec{y} = \sum_i\vec{y}_{(i)}$ where $i$ is
...
$$ \frac{\partial L}{\partial x_j} = \sum_\limits{k} \frac{\partial L}{\partial y_k} \frac{\partial y_j}{\partial x_k} $$

where $\partial y_k$ = $\sum_i\partial y_{k (i)}$. By pulling out this summation over $i$,

$$ \frac{\partial L}{\partial x_j} = \sum_\limits{i} \sum_\limits{k} \frac{\partial L}{\partial y_{k (i)}} \frac{\partial y_j}{\partial x_k} $$
...
that $J_{kj (i)} = \frac{\partial y_{k (i)}}{\partial j_k}$.
